...
export default function EfficientFrontierPage() {
  ...
  const handleCalculateCustomPortfolio = () => {
    ...
    if (selectedAssets.length === 0) return alert(t('simulation.selectAssetsFirst'));
    if (totalWeight < 99.9 || totalWeight > 100.1) return alert(t('simulation.allocationTotalError'));
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col space-y-2">
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">{t('simulation.efTitle')}</h1>
        <p className="text-slate-500 text-lg">{t('simulation.efSubtitle')}</p>
      </div>
