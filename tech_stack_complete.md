...
```
       ...
       /E2E\         Playwright (10%)
      /------\
     /  API   \      FastAPI TestClient (30%)
    /----------\
   /    Unit    \    Vitest + Pytest (60%)
  /--------------\
```
...
import pytest
import numpy as np
from app.calculations.efficient_frontier import calculate_efficient_frontier
...
def test_efficient_frontier_basic():
    ...
    assert all(0 <= p['volatility'] <= 0.5 for p in result)
    assert all(sum(p['weights'].values()) == pytest.approx(1.0) for p in result)
...
```typescript
// e2e/portfolio-workflow.spec.ts
import { test, expect } from '@playwright/test'

test.describe('ポートフォリオ作成フロー', () => {
  test('新規ポートフォリオ作成から効率的フロンティア表示まで', async ({ page }) => {
    // ログイン
    ...
    await page.fill('[name=name]', 'テストポートフォリオ')
    
    // 資産配分設定
    await page.selectOption('[data-testid=asset-select-0]', 'TOPIX')
    await page.fill('[data-testid=weight-input-0]', '60')
    await page.click('[data-testid=add-asset]')
    await page.selectOption('[data-testid=asset-select-1]', 'SP500')
    await page.fill('[data-testid=weight-input-1]', '40')
    
    await page.click('button:has-text("保存")')
    
    // 効率的フロンティア表示確認
    await page.click('text=効率的フロンティア')
    await expect(page.locator('.plotly')).toBeVisible({ timeout: 5000 })
    await expect(page.locator('text=期待リターン')).toBeVisible()
  })
})
```
...
│   ├── poetry.lock
│   └── requirements.txt               # pip用（Railway用）
│
├── data/                              # データ管理
│   ├── scripts/
│   │   ├── seed_assets.py             # 資産マスタ初期投入
│   │   └── update_asset_data.py       # 日次更新スクリプト
│   ├── raw/
│   │   └── asset_data.csv
│   └── processed/
...
- [x] ポートフォリオCRUD API
- [x] 効率的フロンティア計算実装
- [x] Plotlyグラフコンポーネント作成
- [x] ログイン・ダッシュボード画面

**Week 3: 統合・デプロイ**
- [x] フロントエンド↔バックエンド統合
- [x] Vercel + Railway デプロイ
- [x] E2Eテスト追加
- [x] MVP リリース

### Phase 2: コア機能拡張（3-4週間）
...
| サービス | プラン | 月額コスト | 備考 |
|---------|-------|----------|------|
| **Vercel** | Hobby | $0 | 100GB転送、Serverless Functions 100GB-時間 |
| **Railway** | Starter | $5 | 500実行時間、2GB RAM、10GB Storage |
| **Supabase** | Free | $0 | 500MB DB、50K MAU、2GB転送 |
...
| サービス | プラン | 月額コスト | 備考 |
|---------|-------|----------|------|
| **Vercel** | Hobby | $0-20 | 超過時は従量課金 |
| **Railway** | Hobby | $10-20 | 使用量に応じて変動 |
| **Supabase** | Free-Pro | $0-25 | Pro移行で無制限 |
| **合計** | - | **$10-65/月** | - |

### コスト最適化戦略

1. **キャッシング活用:** React Queryで計算結果を積極的にキャッシュ
2. **静的データ化:** 日次バッチで資産データを事前計算→JSON配信
3. **CDN活用:** Cloudflareでグラフ画像をキャッシュ
4. **モニタリング:** Railway のメトリクスで使用量監視
...
```bash
# Supabase
NEXT_PUBLIC_SUPABASE_URL=https://xxxxx.supabase