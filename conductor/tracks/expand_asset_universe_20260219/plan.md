...
## Phase 1: バックエンド - 資産マスタの拡充とデータ収集

- [ ] **Task: シードデータ・リストの定義と拡張**
...
- [ ] **Task: Conductor - User Manual Verification 'バックエンド資産データ拡充' (Protocol in workflow.md)**

## Phase 2: フロントエンド - 資産選択UIのフィルタリング機能

- [ ] **Task: アセットクラス・フィルタリングのロジック実装**
    - [ ] `frontend/src/hooks/useAssets.ts` または関連ストアを修正し、利用可能なアセットクラスのユニークリストを取得できるようにする。
    - [ ] `AssetSelector.tsx` 内で、選択されたフィルタ（例: 「すべて」「株式」「暗号資産」）に基づきリストを絞り込むフィルタリング関数を作成。
- [ ] **Task: UIコンポーネントの作成と統合**
    - [ ] 資産選択リストの上部に、アセットクラスを選択するためのタブ（Tabs）またはボタン・グループを追加。
    - [ ] フィルタ選択時にリストが即座に更新されることを確認。
    - [ ] ユニットテストを作成し、各フィルタが正しく動作することを検証。
