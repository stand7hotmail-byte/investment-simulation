...
## Phase 1: バックエンド - リスクパリティ計算ロジックの実装
...
- [x] Task: コアロジックのテスト通過確認 4b8b12e
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Backend Logic' (Protocol in workflow.md)

## Phase 2: バックエンド - APIエンドポイントとキャッシュの実装
- [ ] Task: リスクパリティ計算用APIエンドポイント (`/simulation/risk-parity`) の統合テスト作成
- [ ] Task: FastAPIによるAPIエンドポイントの実装
- [ ] Task: 計算結果のキャッシュ機構（同一パラメータでの再計算防止）の実装
- [ ] Task: 統合テストの通過確認
- [ ] Task: Conductor - User Manual Verification 'Phase 2: API & Cache' (Protocol in workflow.md)
