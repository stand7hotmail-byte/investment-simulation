...
async def test_auth_es256_unauthorized_due_to_unknown_key():
    ...
    with patch("app.main.jwks_client") as mock_jwks:
        ...
        assert "ES256 signature verification failed" in excinfo.value.detail

@pytest.mark.anyio
async def test_auth_es256_success():
    # This test represents what SHOULD work after the fix
    from cryptography.hazmat.primitives import serialization
    from cryptography.hazmat.primitives.asymmetric import ec
    ...
    with patch("app.main.jwks_client") as mock_jwks:
        ...
        # In the current implementation, this SHOULD succeed if jwks_client works
